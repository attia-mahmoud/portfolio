  ...
  return (
    <Box sx={styles.card}>
      ...
      <Box sx={styles.socialShare} className="social__share">
        {social.map((item) => (
          <Link key={item.id} target="_blank" rel="noreferrer" href={item.path} className={item.name} ariaLabel={item.name}>
              {item.icon}
          </Link>
        ))}
      </Box>
...
const styles = {
  ...
  memberThumb: {
    width: ['100px', '80px', '100px', null, null, '160px'],
    height: ['100px', '80px', '100px', null, null, '160px'],
    flexShrink: 0,
    border: '2px solid',
    borderColor: '#000000',
    borderRadius: '50%',
